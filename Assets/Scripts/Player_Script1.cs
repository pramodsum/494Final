...
public class Player_Script1 : MonoBehaviour
{
		...
		private bool isRevPressed = false;
		...
		void Update ()
		{
				...
				if ((Input.GetKeyDown (KeyCode.R) && playerNum == 1) || 
...
//		vect.z = 0;
				...
				if (!outOfBounds) {//Do we want to allow the rotations if no engine?
						...
						if (isRevPressed) {
								Vector3 directionVector = (camera.position - transform.position).normalized;
								rigidbody.AddForce (directionVector * forceModifier);
						}
